...
Frontend -> Frontend: <color #ff00ff>【可选：】</color><color #6CA6CD>计算real challenge: H (backend challenge + email + "register_action")</color>
User <- Frontend: 请求注册passkey
User -> Frontend: 选择Google 密码管理，进行生物识别注册passkey
Frontend-> Backend: 传递注册的cred + challenge信息到服务器
Backend->Backend: cred 落库<color #6CA6CD>（超时删除？）</color>
end
