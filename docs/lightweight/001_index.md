...
```tsx
import { createChart, ColorType, LineSeries, createTextWatermark, ChartOptions, DeepPartial, AreaSeries } from 'lightweight-charts';
...
export const ChartComponent: React.FC<ChartComponentProps> = (props) => {
    ...
    }, [data, backgroundColor, lineColor, textColor, areaTopColor, areaBottomColor, chartOptions]);
...
export const LightWeightTV: React.FC = (props) => {
    const [interval, setInterval] = useState('1D')

    const seriesesData = useMemo(() => new Map([
        ['1D', dayData],
        ['1W', weekData],
        ['1M', monthData],
        ['1Y', yearData],
    ...
    const data = useMemo(() => seriesesData.get(interval || '1D'), [interval])
    return <div style={{ width: "100%", height: 400, paddingBottom: 40 }}>
        <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
            <div>XXX coin</div>
            <div style={{ display: "flex", gap: 4, zIndex: 999 }}>{intervals.map(item => <button key={item} style={{ color: interval === item ? '#1f0' : '' }} onClick={() => setInterval(item)}>{item}</button>)}</div>
        </div>

        <ChartComponent {...props} data={data!} />
    </div>;
};
